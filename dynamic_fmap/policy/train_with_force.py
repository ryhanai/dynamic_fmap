'''
Default data conversion command (from ManiSkill Deffusion Policy)
...
'''
Default training command (from ManiSkill Deffusion Policy)
seed=1
demos=100
...
def convert_obs(obs, concat_fn, transpose_fn, state_obs_extractor, depth = True):
    ...
    if "force_camera" in img_dict:
        out_dict["point_forces"] = img_dict["force_camera"]["point_forces"]
...
class Agent(nn.Module):
    def __init__(self, env: VectorEnv, args: Args):
        ...
        # self.include_rgb = args.obs_mode in ["rgb"]
        ...
        self.noise_pred_net = ConditionalUnet1D(
            ...
            global_cond_dim=self.obs_horizon * (force_feature_dim + obs_state_dim),            
        ...
        self.point_force_encoder = SmallSetTransformerEncoder(
            in_feat_dim=3,
            model_dim=128,
            heads=4,
            blocks=2,
            out_dim=force_feature_dim,
            use_dist_feat=False,
            drop=0.0,
        )
    ...
    def encode_obs(self, obs_seq, eval_mode):
        ...
        if self.include_force:
            ...
            point_force_feature = self.point_force_encoder(x_pts, x_feats)  # (B*obs_horizon, G)
            point_force_feature = point_force_feature.reshape(
                batch_size, self.obs_horizon, point_force_feature.shape[1]
            )  # (B, obs_horizon, G)
            feature = torch.cat(
                (point_force_feature, obs_seq["state"]), dim=-1
            )  # (B, obs_horizon, G+obs_state_dim)
            return feature.flatten(start_dim=1)  # (B, obs_horizon * (G+obs_state_dim))
