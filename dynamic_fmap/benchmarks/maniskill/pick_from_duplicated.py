...
@register_env("PickDuplicated-v1", max_episode_steps=50)
class PickDuplicatedEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def _load_mesh(self, name: str, filename, pose: list):
        builder = self.scene.create_actor_builder()
        builder.add_convex_collision_from_file(filename=filename)
        builder.add_visual_from_file(filename=filename)
        builder.initial_pose = sapien.Pose(p=pose)
        mesh_actor = builder.build(name=name)

        return mesh_actor

        # # YCB mesh
        # builder.add_convex_collision_from_file(
        #     filename="/home/ryo/Dataset/ycb_conveni/ycb/010_potted_meat_can/google_16k/textured.obj"
        # )
